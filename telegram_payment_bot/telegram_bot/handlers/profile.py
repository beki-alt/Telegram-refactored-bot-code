...
async def _send_profile(query, user: dict) -> None:
    ...
    if user.get("joined_at"):
        try:
            ...
            logger.warning(f"joined_at parse failed for user {tg_id}: {exc}")
            joined_display = "—"   # never show raw GC date on failure

    cycle       = db.get_billing_cycle()   # always read fresh — reflects admin changes instantly
...
def build_profile_conversation() -> ConversationHandler:
    ...
    Entry points: profile_edit_name, profile_edit_phone, and profile_refresh callbacks.
    BUG-5 FIX: Added profile_refresh and profile_view to entry_points so the
    🔄 Refresh button works even when no edit conversation is active.
    """
    return ConversationHandler(
        entry_points=[
            CallbackQueryHandler(
                profile_callback,
                pattern=r"^profile_(edit_name|edit_phone|view|refresh)$",
            ),
        ],
        states={
            EDIT_NAME_INPUT: [
