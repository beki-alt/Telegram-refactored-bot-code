...
📅 Payment Schedule — billing cycle countdown.

Bug fixes from original:
  - register_user() called without phone arg (fixed — phone optional).
  - create_payment_record() now passes receipt_file_id (the Telegram file_id)
    so the admin inbox can call send_photo(photo=file_id) rather than
    send_photo(photo=message_id) which crashes.
"""
...
def _resolve_receipt_chat_id() -> int | None:
...
async def pay_renew(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Entry point: show bank account details or inform if already paid."""
...
async def confirm_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_payment":
        ...
        # Forward receipt to the admin/receipt channel
        try:
            ...
            logger.error(
                f"Failed to forward receipt for user {tg_id}:\n{traceback.format_exc()}"
            )
        ...
        # FIX: store both channel message_id AND the file_id so admin inbox
        # can display the photo using send_photo(photo=file_id) rather than
        # the broken send_photo(photo=message_id) from the original.
        ...
        await query.message.reply_text(T.BACK_TO_MENU, reply_markup=main_menu_keyboard())
        logger.info(f"User {tg_id} submitted receipt (channel msg {channel_msg_id}).")
        return ConversationHandler.END
...
async def payment_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if today >= start:
        ...
        if end < start:  # cross-month cycle (e.g. 25→5)
    ...
    user        = db.get_user(update.effective_user.id)
    user_status = T.STATUS_PAID if user and user.get("status") == "paid" else T.STATUS_UNPAID
    ...
    return ConversationHandler(
        ...
        states={
            PAYMENT_AWAITING_SCREENSHOT: [
                ...
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND, receive_payment_screenshot
                ),
            ],
            PAYMENT_CONFIRM: [
                CallbackQueryHandler(
