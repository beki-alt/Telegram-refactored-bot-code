...
from texts import T
from utils import eth_month_name, prev_eth_months
...
def billing_start_day_keyboard(today_day: int, current_start_day: int) -> InlineKeyboardMarkup:
    """
    Day-picker for the billing start day.

    Shows buttons for every day from today_day through 30.
    The button matching current_start_day gets a ✓ suffix.
    Layout: up to 5 buttons per row, followed by a ✅ Done row.
    """
    days = list(range(today_day, 31))
    rows = []
    for i in range(0, len(days), 5):
        row = []
        for d in days[i:i + 5]:
            label = f"{d} ✓" if d == current_start_day else str(d)
            row.append(InlineKeyboardButton(label, callback_data=f"bill_start_pick_{d}"))
        rows.append(row)
    rows.append([InlineKeyboardButton(T.BTN_DAY_DONE, callback_data="bill_start_done")])
    rows.append([InlineKeyboardButton(T.BTN_BACK,     callback_data="adm_billing_cycle")])
    return InlineKeyboardMarkup(rows)


def billing_end_day_keyboard(
    today_day: int,
    start_day: int,
    current_end_day: int,
    current_is_next_month: bool,
) -> InlineKeyboardMarkup:
    """
    Day-picker for the billing end day.

    Same-month section : today_day → 30  (callback bill_end_pick_{d})
    Next-month section : 1 → 5           (callback bill_end_next_{d}, label →1 … →5)

    The button matching (current_end_day, current_is_next_month) gets a ✓ suffix.
    Layout: up to 5 per row per section, followed by a ✅ Done row.
    """
    rows = []

    # ── Same-month days ───────────────────────────────────────────────────────
    same_days = list(range(today_day, 31))
    for i in range(0, len(same_days), 5):
        row = []
        for d in same_days[i:i + 5]:
            is_current = (d == current_end_day and not current_is_next_month)
            label = f"{d} ✓" if is_current else str(d)
            row.append(InlineKeyboardButton(label, callback_data=f"bill_end_pick_{d}"))
        rows.append(row)

    # ── Next-month days (1–5) ─────────────────────────────────────────────────
    next_row = []
    for d in range(1, 6):
        is_current = (d == current_end_day and current_is_next_month)
        label = f"→{d} ✓" if is_current else f"→{d}"
        next_row.append(InlineKeyboardButton(label, callback_data=f"bill_end_next_{d}"))
    rows.append(next_row)

    rows.append([InlineKeyboardButton(T.BTN_DAY_DONE, callback_data="bill_end_done")])
    rows.append([InlineKeyboardButton(T.BTN_BACK,     callback_data="adm_billing_cycle")])
    return InlineKeyboardMarkup(rows)


def bank_keyboard(accounts: list | None = None) -> InlineKeyboardMarkup:
    """Show delete buttons for every active account, then Add and Back."""
    rows = []
    for a in (accounts or []):
        label = T.BTN_DELETE_BANK.format(bank_name=a["bank_name"])
        rows.append([InlineKeyboardButton(label, callback_data=f"bank_del_{a['id']}")])
    rows.append([InlineKeyboardButton(T.BTN_ADD_BANK, callback_data="bank_add")])
    rows.append([InlineKeyboardButton(T.BTN_BACK,     callback_data="adm_settings")])
    return InlineKeyboardMarkup(rows)
...
def manual_action_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(T.BTN_BACK,        callback_data="adm_users")],
    ])
...
def receipt_action_keyboard(payment_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(T.BTN_BACK, callback_data="adm_inbox")],
    ])
