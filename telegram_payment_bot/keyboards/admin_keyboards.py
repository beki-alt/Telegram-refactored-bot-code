...
Phase 5 addition: day_picker_keyboard() — builds a grid of 30 day buttons
  (6 rows × 5 columns) for button-based billing-cycle day selection.
  The currently selected day is marked with ✅.
"""
...
def notify_toggle_keyboard(notification_keys: dict, get_setting_fn) -> InlineKeyboardMarkup:
    ...
    for key, label in notification_keys.items():
        ...
        rows.append([InlineKeyboardButton(
            f"{label} — {status}", callback_data=f"toggle_{key}"
        )])
    rows.append([InlineKeyboardButton(T.BTN_BACK, callback_data="adm_settings")])
    return InlineKeyboardMarkup(rows)
...
def day_picker_keyboard(
    ...
    """
    Phase 5: build a grid of day buttons (1–max_day) with ✅ on current_day.
    Layout: 6 rows × 5 columns, plus a Back row.
    callback_data = "{callback_prefix}_{day}"
    """
    rows = []
    cols = 5
    days = list(range(1, max_day + 1))

    for i in range(0, len(days), cols):
        row = []
        for day in days[i:i + cols]:
            label = f"✅ {day}" if day == current_day else str(day)
            row.append(InlineKeyboardButton(
                label, callback_data=f"{callback_prefix}_{day}"
            ))
        rows.append(row)
...
def billing_trigger_keyboard() -> InlineKeyboardMarkup:
    """Phase 6: prompt to immediately send payment-start notification."""
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(T.BTN_SEND_NOW,  callback_data="billing_trigger_now"),
            InlineKeyboardButton(T.BTN_SKIP_SEND, callback_data="billing_trigger_skip"),
        ]
    ])
...
def support_reply_keyboard(msg_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(
            T.BTN_REPLY_MSG.format(msg_id=msg_id),
            callback_data=f"reply_sup_{msg_id}",
        )]
    ])
...
def month_picker_keyboard(prefix: str, n: int = 6) -> InlineKeyboardMarkup:
    """Last n Ethiopian months, 2 per row."""
    ...
    for i in range(0, len(months), 2):
        ...
        for y, m in months[i:i + 2]:
            ...
            row.append(InlineKeyboardButton(
                label, callback_data=f"{prefix}_{y}_{m}"
            ))
        rows.append(row)
