...
class T:
    ...
    REGISTER_SUCCESS    = "✅ መረጃዎ ተመዝግቧል።"
    REGISTER_PHONE_PROMPT = "📱 እባክዎ ስልክ ቁጥርዎን ያጋሩ።"
    REGISTER_NAME_PROMPT  = "👋 እንኳን ደህና መጡ!\n\nእባክዎ ሙሉ ስምዎን ያስገቡ።"
    ...
    ASK_PHONE = (
        ...
        "ከታቹ ያለውን *«📱 ስልክ ቁጥር አጋራ»* ቁልፍ ይጫኑ:"
    )
    ASK_PHONE_BUTTON_ONLY = (
        "⚠️ እባክዎ ቁልፉን ይጫኑ — ስልክ ቁጥር አይተይቡ።\n\n"
        "📱 «ስልክ ቁጥር አጋራ» ቁልፉን ይጠቀሙ:"
    )
    PHONE_RECEIVED = (
        "✅ *ስልክ ቁጥርዎ ተመዝግቧል!*\n\n"
        "📱 ቁጥር: `{phone}`\n\n"
        "ምዝገባዎ ተጠናቋል። እንኳን ደህና መጡ, {name}! 🎉"
    )

    UNKNOWN_COMMAND = "❓ ምርጫዎን ከምናሌ ያድርጉ።\n/start ብለው ምናሌ ያሳዩ።"
    ...
    EDIT_PHONE_PROMPT  = "📱 *ስልክ ቀይር*"
    EDIT_PHONE_SHARE   = "ከታቹ ያለውን ቁልፍ ይጫኑ፣ ወይም አዲሱን ስልክ ቁጥር ያስገቡ:"
    EDIT_PHONE_INVALID = "❌ ያስገቡት ቁጥር ትክክል አይደለም። እባክዎ እንደገና ያስገቡ:"
    ...
    PAYMENT_PHOTO_ONLY = "❌ *ፎቶ ብቻ ይቀበላሉ!*\n\nደረሰኝ ፎቶዎን ያስቀምጡ:"
    ...
    # ─── Phase 5: billing cycle now shows current values and uses button picker ─
    ...
    # Phase 5: day picker prompt (button-based — no text input)
    BILLING_PICK_START_HEADER = (
        "📅 *የጅምር ቀን ምርጥ*\n\n"
        "አሁን: *{current}ኛ* ✅\n\n"
        "አዲሱን ቀን ይምረጡ:"
    )
    BILLING_PICK_END_HEADER = (
        "📅 *የማብቂያ ቀን ምርጥ*\n\n"
        "አሁን: *{current}ኛ* ✅\n\n"
        "አዲሱን ቀን ይምረጡ:"
    )
    ...
    # ─── Phase 6: immediate start-day trigger ────────────────────────────────
    BILLING_START_TODAY_PROMPT = (
        "⚠️ *ዛሬ አዲሱ የጅምር ቀን ({day}ኛ) ነው!*\n\n"
        "የክፍያ ጊዜ ጅምር ማሳወቂያ አሁን ልካቸው?"
    )
    BTN_SEND_NOW   = "✅ አዎ — አሁን ልካቸው"
    BTN_SKIP_SEND  = "❌ አይ — ኋላ ልካቸው"
    BILLING_START_SENT_NOW  = "✅ የክፍያ ጅምር ማሳወቂያ ወዲያውኑ ተልኳል!"
    BILLING_START_SKIPPED   = "⏭️ ማሳወቂያ ዘለሏል — ሁሉም ልካቸው ይሆናሉ።"
    ...
    DEFAULT_MSG_PAYMENT_START = (
        ...
        "ከ{start_day}ኛ እስከ {end_day}ኛ ባለው ጊዜ ውስጥ ክፍያዎን እንዲፈጽሙ ጥሪ እናቀርባለን።\n\n"
        "💳 ለክፍያ መመሪያ ዋናውን ምናሌ ይጠቀሙ።"
    )
    DEFAULT_MSG_ONE_DAY = (
        "⚠️ ትዝታ!\n\n"
        "ነገ {end_day}ኛ — የደንበኝነት ክፍያ የሚቆምበት ቀን ነው። "
        "ገና ካልከፈሉ፣ ዛሬ ክፍያዎን ፈጽሙ!\n\n"
        "💳 'ክፈል/አድስ' የሚለውን ምናሌ ይጠቀሙ።"
    )
    DEFAULT_MSG_FINAL_DAY = (
        "🚨 የመጨረሻ ቀን!\n\n"
        "ዛሬ {end_day}ኛ — የደንበኝነት ክፍያ የመጨረሻ ቀን ነው። "
        "ገና ካልከፈሉ ወዲያውኑ ይፈጽሙ!\n\n"
    ...
    # ──────────────────────────────────────────────────────────────────────────
    MISSED_JOB_NOTICE = (
        "⚠️ *ቦቱ ጊዜ ያለፈ ማሳወቂያ ደርሷል*\n\n"
        "ቦቱ ጊዜ ውስጥ ሳይሰራ ቆሟል። ያለፈው {job_name} ዛሬ ተልኳል።"
    )
