...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Bank accounts ─────────────────────────────────────────────────────────
        ...
        if accounts:
            ...
            text += T.BANK_NONE
        await query.edit_message_text(
            text, reply_markup=bank_keyboard(accounts), parse_mode="Markdown"
        )

    elif data.startswith("bank_del_"):
        try:
            acct_id = int(data.split("_")[-1])
        except ValueError:
            await query.answer(T.BANK_DELETE_FAIL, show_alert=True)
            return
        # Find the account name before deleting (for the confirmation message)
        accounts_before = db.get_active_bank_accounts()
        acct = next((a for a in accounts_before if a["id"] == acct_id), None)
        ok = db.deactivate_bank_account(acct_id)
        if ok:
            bank_name = acct["bank_name"] if acct else str(acct_id)
            logger.info(f"Admin deactivated bank account id={acct_id} ({bank_name}).")
            accounts_after = db.get_active_bank_accounts()
            text = T.BANK_HEADER
            if accounts_after:
                ...
                text += T.BANK_NONE
            await query.answer(T.BANK_DELETED.format(bank_name=bank_name))
            await query.edit_message_text(
                text, reply_markup=bank_keyboard(accounts_after), parse_mode="Markdown"
            )
        else:
            await query.answer(T.BANK_DELETE_FAIL, show_alert=True)
