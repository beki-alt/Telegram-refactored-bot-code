...
FIX: All state constants imported from states.py (globally unique integers 20-25).
"""
...
async def receive_billing_end(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Warn if end < start (creates a cross-month window which may confuse users)
    start = db.get_billing_cycle()["start"]
    if day < start:
        await update.message.reply_text(
            T.BILLING_END_INVALID_ORDER.format(end=day, start=start),
            parse_mode="Markdown",
        )
        return SET_BILLING_END
