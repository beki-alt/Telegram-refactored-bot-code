...
from texts import T
from utils import eth_month_name, eth_storage_to_display
...
async def report_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "report_quick":
        ...
        # Compute end_month_name: for cross-month windows (start > end) the end
        # day falls in the month AFTER mo; same-month windows end in mo itself.
        is_cross    = cycle["start"] > cycle["end"]
        end_month_n = (1 if mo == 13 else mo + 1) if is_cross else mo
        ...
        for user in unpaid:
            try:
                msg = template.format(
                    ...
                    end_month_name = eth_month_name(end_month_n),
                )
            ...
            # Rate-limit protection: ~20 msg/s — well under Telegram's 30 msg/s limit
            await _asyncio.sleep(0.05)

        await query.edit_message_text(
            T.NOTIFY_DONE.format(
                month_name = month_label,
                year       = yr,
                sent       = sent,
                failed     = failed,
        ...
        logger.info(f"Unpaid notification: month={mo}/{yr}, sent={sent}, failed={failed}.")
    ...
    try:
        ...
        for i, p in enumerate(payments, 1):
            ...
            rows.append({
                ...
                "ሁኔታ":       p["status"],
                "ቀን":         eth_storage_to_display(p.get("eth_payment_date", "")),
