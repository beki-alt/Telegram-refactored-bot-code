...
async def send_payment_start_reminder(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Record the date this reminder fired so cross-month guards (_one_day_guard,
    # _final_day_guard) can confirm the billing window is currently open before
    # sending reminders.  Without this, those guards fire twice per cycle when
    # start > end (cross-month configuration).
    eth_year, eth_month, eth_day = to_ethiopian(now_eth())
    today_str = f"{eth_year}-{eth_month:02d}-{eth_day:02d}"
    db.set_setting("last_payment_start_eth_date", today_str)
    logger.info(f"Recorded last_payment_start_eth_date = {today_str}.")
